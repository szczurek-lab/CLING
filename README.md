...
print(fitted.K)                              # number of active factors
...
- Factor selection: a factor is kept if its per-view explained variance is at least `epsilon = 0.01` in one or more views, applied after the ELBO converges. The active count is reported as `fitted.K`.

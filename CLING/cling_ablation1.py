######################## CLING-Ablation1 ########################
# Variant with NO phi node: removes feature-level Gamma prior coupling; alpha prior is Ga(a_alpha, b_alpha).
...
__all__ = ["ClingFA_ablation1", "compute_init_K"]
...
class nodeFA_alpha_m(nodeFA_general):
    ...
    Prior (NO-phi): α_{d,k}^{(m)} ~ Ga(a_α, b_α)
    """
    ...
    def MB(self, w_m_node, delta_m_node):
        self.w_m_node = w_m_node; self.delta_m_node = delta_m_node

    def update(self):
        E_gamma = self.delta_m_node.E_gamma[np.newaxis, :]
        E_W2 = self.w_m_node.E_w_squared
        self.vi_a = np.full((self.Dm, self.K), self.a_alpha + 0.5, dtype=float)
        self.vi_b = self.b_alpha + 0.5 * (E_gamma * E_W2)
        self.update_params()
    ...
    def update_k(self, k):
        ...
        self.vi_b[:, k] = self.b_alpha + 0.5 * E_gamma_k * E_W2_k
    ...
    def ELBO(self):
        ...
        logp = a0 * (np.log(b0 + EPS)) + (a0 - 1) * E_log_alpha - b0 * E_alpha - gammaln(a0)
...
class ClingFA_ablation1:
    ...
    def _wire_MB(self):
        ...
        for m in range(self.M):
            ...
            self.nodelist_alpha[m].MB(self.nodelist_w[m], self.nodelist_delta[m])
    ...
    def add_factors(self, k_new: int = 1, init_scale: float = 1e-2,
        ...
        for m in range(M):
            ...
            vi_b_new_base = np.full((D[m], 1), a.b_alpha) + 1e-3  # (D_m, 1)
    ...
    def from_numpy_views(views, K=None, center=None,
        ...
        return ClingFA_ablation1(data=data, N=N, M=M, K=K, D=D,

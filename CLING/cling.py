######################## CLING ########################
# Cross-view Latent Integration via Nonparametric Gamma-Process Factor Analysis
...
__all__ = ["ClingFA", "compute_init_K"]
...
def compute_init_K(D: List[int], M: int, mode: str = "paper", c: float = 5.0,
    ...
    """
    Choose initial K.
    mode='paper'  -> K0 = ceil(c * log(sum D_m))               # mirrors 5*log p
    mode='Mlogmax'-> K0 = ceil(M * log(max D_m))           
    """
    p = int(np.sum(D))
    dmax = int(np.max(D))
    if mode == "Mlogmax":
...
class nodeFA_w_m(nodeFA_general):
    ...
    Prior: W_{d,k}^{(m)} ~ N(0, 1 / (alpha_{d,k}^{(m)} * gamma_k^{(m)}))
    """
    ...
    def MB(self, alpha_m_node, z_node, y_m_node, tau_m_node, delta_m_node):
        self.alpha_m_node = alpha_m_node
        self.z_node = z_node
        self.y_m_node = y_m_node
        self.tau_m_node = tau_m_node
        self.delta_m_node = delta_m_node
        self.update_params()
        self.update_params_z()

    def update_k(self, k):
        E_tau = self.tau_m_node.E_tau
        E_alpha_k = self.alpha_m_node.E_alpha[:, k]
        E_gamma_k = self.delta_m_node.E_gamma[k]
        Z = self.z_node.E_z
        Zk = Z[:, k]
        Y = self.y_m_node.Y
        Msk = self.y_m_node.M
        ...
        nominator = Zk @ partial
        sum_z2_masked = (Msk * self.z_node.E_z_squared[:, k][:, None]).sum(axis=0)
        prior_prec = E_alpha_k * E_gamma_k
    ...
    def ELBO(self):
        E_log_alpha = self.alpha_m_node.E_log_alpha
        E_log_gamma_k = self.delta_m_node.E_log_gamma
        E_alpha = self.alpha_m_node.E_alpha
        E_gamma = self.delta_m_node.E_gamma
        E_log_alpha_gamma = E_log_alpha + E_log_gamma_k[np.newaxis, :]
        E_alpha_gamma = E_alpha * E_gamma[np.newaxis, :]
        logp = 0.5 * np.sum(E_log_alpha_gamma) - 0.5 * np.sum(E_alpha_gamma * self.E_w_squared)
        entropy = 0.5 * self.D[self.m]*self.K + 0.5 * np.sum(log_eps(self.vi_var, EPS))
        self.elbo = logp + entropy


# ------------------------ MGP delta ------------------------
class nodeMGP_delta(nodeFA_general):
    def __init__(self, a1, b1, a2, b2, m, K, general_params):
        super().__init__(**general_params)
    ...
    def MB(self, node_w_m, node_alpha_m):
        self.node_w_m = node_w_m
        self.node_alpha_m = node_alpha_m

    def _ak_bk(self, k): return (self.a1, self.b1) if k == 0 else (self.a2, self.b2)

    def update_k(self, k, E_W_sq_all, E_alpha_all):
        a_k, b_k = self._ak_bk(k)
        self.vi_a[k] = a_k + ((self.K - k) * self.D[self.m]) / 2.0
        rate = b_k
        for j in range(k, self.K):
            sum_d = np.sum(E_alpha_all[j, :] * E_W_sq_all[j, :])
            E_gamma_j = self.E_gamma[j]
            rate += 0.5 * (E_gamma_j / max(self.E_delta[k], 1e-12)) * sum_d
        self.vi_b[k] = rate

    def update_all(self, E_W_sq_all, E_alpha_all):
        for k in range(self.K):
            self.update_k(k, E_W_sq_all, E_alpha_all)
        self.update_params()
...
# ------------------------ phi ------------------------
class nodeFA_phi_m(nodeFA_general):
    """
    q(φ^{(m)}_d) = Gamma(vi_a_phi[d], vi_b_phi[d])
    Prior: φ_d^{(m)} ~ Ga(a_φ, b_φ)
    α_{d,k}^{(m)} ~ Ga(a_α, b_α * φ_d^{(m)})
    """
    def __init__(self, a_phi, b_phi, b_alpha, m, general_params):
        super().__init__(**general_params)
        self.m = m
        self.a_phi = float(a_phi); self.b_phi = float(b_phi); self.b_alpha = float(b_alpha)
        Dm = self.D[self.m]
        self.vi_a_phi = np.full(Dm, self.a_phi, dtype=float)
        self.vi_b_phi = np.full(Dm, self.b_phi, dtype=float)
        self.elbo = 0.0
        self.update_params()

    def MB(self, alpha_m_node):
        self.alpha_m_node = alpha_m_node

    def update(self):
        Dm = self.D[self.m]
        a_alpha = self.alpha_m_node.a_alpha
        sum_E_alpha_d = np.sum(self.alpha_m_node.E_alpha, axis=1)
        self.vi_a_phi = np.full(Dm, self.a_phi + a_alpha * self.K, dtype=float)
        self.vi_b_phi = self.b_phi + self.b_alpha * sum_E_alpha_d
        self.update_params()

    def update_params(self):
        self.vi_b_phi = np.maximum(self.vi_b_phi, 1e-8)
        min_shape = 1.0001
        vi_a_safe = np.maximum(self.vi_a_phi, min_shape)
    ...
    def ELBO(self):
        ...
        E_log_phi = self.E_log_phi; E_phi = self.E_phi
        logp = a0 * np.log(b0 + EPS) + (a0 - 1) * E_log_phi - b0 * E_phi - gammaln(a0)
        logq = a * np.log(b + EPS) + (a - 1) * E_log_phi - b * E_phi - gammaln(a)
        self.elbo = np.sum(logp - logq)


# ------------------------ alpha ------------------------
class nodeFA_alpha_m(nodeFA_general):
    """
    q(α^{(m)}_{d,k}) = Ga(vi_a[d,k], vi_b[d,k]),
    Prior: α_{d,k}^{(m)} ~ Ga(a_α, b_α * φ_{d}^{(m)})
    """
    def __init__(self, a_alpha, b_alpha, m, general_params):
        ...
        self.vi_b = np.full((self.Dm, self.K), max(self.b_alpha, 1e-8), dtype=float)
        self.update_params(); self.elbo = 0.0

    def MB(self, w_m_node, delta_m_node, phi_m_node):
        self.w_m_node = w_m_node; self.delta_m_node = delta_m_node; self.phi_m_node = phi_m_node

    def update(self):
        E_gamma = self.delta_m_node.E_gamma[np.newaxis, :]
        E_W2 = self.w_m_node.E_w_squared
        E_phi_d = self.phi_m_node.E_phi[:, np.newaxis]
        self.vi_a = np.full((self.Dm, self.K), self.a_alpha + 0.5, dtype=float)
        self.vi_b = (self.b_alpha * E_phi_d) + 0.5 * (E_gamma * E_W2)
        self.update_params()

    def update_k(self, k):
        E_gamma_k = self.delta_m_node.E_gamma[k]
        E_W2_k = self.w_m_node.E_w_squared[:, k]
        E_phi_d = self.phi_m_node.E_phi
        self.vi_a[:, k] = self.a_alpha + 0.5
        self.vi_b[:, k] = (self.b_alpha * E_phi_d) + 0.5 * E_gamma_k * E_W2_k
    ...
    def ELBO(self):
        ...
        E_log_phi = self.phi_m_node.E_log_phi[:, np.newaxis]
        E_phi = self.phi_m_node.E_phi[:, np.newaxis]
        E_log_alpha = self.E_log_alpha; E_alpha = self.E_alpha
        logp = a0 * (np.log(b0 + EPS) + E_log_phi) + (a0 - 1) * E_log_alpha - (b0 * E_phi) * E_alpha - gammaln(a0)
        logq = self.vi_a * log_eps(self.vi_b, EPS) + (self.vi_a - 1) * E_log_alpha - self.vi_b * E_alpha - gammaln(self.vi_a)
        self.elbo = np.sum(logp - logq)
...
class ClingFA:
    def __init__(self, data: Dict[str, np.ndarray], N: int, M: int, K: int, D: List[int],
                 ...
                 a_phi: float = 1.0, b_phi: float = 1.0,
        ...
        self.a_phi, self.b_phi = float(a_phi), float(b_phi)
        ...
        self.nodelist_phi: List[nodeFA_phi_m] = []
        self.nodelist_tau: List[nodeFA_tau_m] = []
        self.nodelist_delta: List[nodeMGP_delta] = []
        ...
        for m in range(self.M):
            ...
            # phi
            node_phi_m = nodeFA_phi_m(a_phi=self.a_phi, b_phi=self.b_phi,
                                      b_alpha=self.ard_b_alpha, m=m, general_params=general_params)
            self.nodelist_phi.append(node_phi_m)

            # delta
            node_delta_m = nodeMGP_delta(a1=self.mgp_a1, b1=self.mgp_b1,
                                         a2=self.mgp_a2, b2=self.mgp_b2,
                                         m=m, K=self.K, general_params=general_params)
            self.nodelist_delta.append(node_delta_m)
    ...
    def _wire_MB(self):
        ...
        for m in range(self.M):
            ...
            self.nodelist_w[m].MB(self.nodelist_alpha[m], self.node_z,
                                  self.nodelist_y[m], self.nodelist_tau[m], self.nodelist_delta[m])
            self.nodelist_alpha[m].MB(self.nodelist_w[m], self.nodelist_delta[m], self.nodelist_phi[m])
            self.nodelist_tau[m].MB(self.nodelist_y[m], self.nodelist_w[m], self.node_z)
            self.nodelist_delta[m].MB(self.nodelist_w[m], self.nodelist_alpha[m])
            self.nodelist_phi[m].MB(self.nodelist_alpha[m])
    ...
    def _update_once(self):
        ...
        # 4) alpha
        for m in range(self.M): self.nodelist_alpha[m].update()

        # 5) phi
        for m in range(self.M): self.nodelist_phi[m].update()

        # 6) delta (MGP)
        for m in range(self.M):
            node_w_m = self.nodelist_w[m]
            node_alpha_m = self.nodelist_alpha[m]
            node_delta_m = self.nodelist_delta[m]
            E_W_sq_all = node_w_m.E_w_squared.T
            E_alpha_all = node_alpha_m.E_alpha.T
            node_delta_m.update_all(E_W_sq_all, E_alpha_all)
    ...
    def _ELBO_once(self):
        ...
        for m in range(self.M):
            ...
            self.nodelist_phi[m].ELBO()
            self.nodelist_y[m].ELBO()
            self.nodelist_tau[m].ELBO()
            self.nodelist_delta[m].ELBO()
            total += ( self.nodelist_w[m].elbo + self.nodelist_alpha[m].elbo
                     + self.nodelist_phi[m].elbo + self.nodelist_y[m].elbo
                     + self.nodelist_tau[m].elbo + self.nodelist_delta[m].elbo )
        self.elbo = total
        return total
    ...
    def add_factors(self, k_new: int = 1, init_scale: float = 1e-2,
        ...
        for m in range(M):
            ...
            a.K = newK
            vi_a_new = np.full((D[m], k_new), a.a_alpha + 0.5)
            vi_b_new_base = (a.b_alpha * self.nodelist_phi[m].E_phi)[:, None] + 1e-3  # (D_m, 1)
            vi_b_new = np.repeat(vi_b_new_base, k_new, axis=1)                        # (D_m, k_new)
            a.vi_a = np.hstack([a.vi_a, vi_a_new])
            a.vi_b = np.hstack([a.vi_b, vi_b_new])
            a.update_params()

            # DELTA (MGP)
            d = self.nodelist_delta[m]
            d.K = newK
            vi_a_new_d = np.full(k_new, d.a2)
            vi_b_new_d = np.full(k_new, d.b2)
            d.vi_a = np.hstack([d.vi_a, vi_a_new_d])
            d.vi_b = np.hstack([d.vi_b, vi_b_new_d])
            d.update_params()
    ...
    def _apply_active_mask(self, active: np.ndarray, verbose: bool = True, msg: str = ""):
        ...
        for m in range(self.M):
            ...
            d = self.nodelist_delta[m]
            d.K = newK
            d.vi_a = d.vi_a[active]
            d.vi_b = d.vi_b[active]
            d.update_params()
    ...
    def _two_value_suffix_min(seq: List[int], max_len: int = 12, min_len: int = 4) -> Tuple[bool, Optional[int]]:
        """
        Scan seq from the end; take the longest contiguous suffix containing ≤2 unique values.
        If it has exactly two unique values and length ≥ min_len, return (True, smaller_of_two).
        Else (False, None).
        """
        if not seq:
            return False, None
    ...
    def fit(self, max_iter: Optional[int] = None, tol: float = 1e-3,
        ...
        elbos: List[float] = []
        K_list: List[int] = []
        egamma_list = [[] for _ in range(self.M)]
        last_elbo = None
        ...
        for it in itertools.count():
            ...
            if it % 10 == 0:
                for m in range(self.M):
                    egamma_list[m].append(self.nodelist_delta[m].E_gamma.copy())
    ...
    def from_numpy_views(views, K=None, center=None,
                         ...
                         prune_min_views=1):
        """
        Build a CLING-FA model from a list of numpy views [N x D_m].
        NaNs are allowed. If K is None, choose automatically with compute_init_K.
        init_mode = "random" or "pca".
        """
        ...
        return ClingFA(data=data, N=N, M=M, K=K, D=D,

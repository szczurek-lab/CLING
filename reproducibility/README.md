...
Scripts, configuration, and data sources for the analyses in the CLING paper.
They fit CLING at the paper's operating point and report the effective factor
count and per-view variance explained. Baseline comparisons and figure assembly
follow the paper's Methods. No data or generated results are committed. Every
dataset is downloaded from its public source.
...
R². The parameter sweeps (factor count, noise, sparsity, sample size) and the 25
seeds are listed in `configs/simulation.yaml`.
